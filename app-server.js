...
export function createBabyCamServer(options = {}) {
  ...
  app.get("/api/config", (_req, res) => {
    const localUrls = getLocalUrls(config.port);
    res.json({
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("host:join", (payload = {}, ack = () => {}) => {
      ...
      leaveCurrentRoom(socket, rooms, io);
    ...
    socket.on("viewer:join", (payload = {}, ack = () => {}) => {
      ...
      const roomState = room || getOrCreateRoom(rooms, roomId);
    ...
    socket.on("signal:offer", ({ to, description } = {}) => {
      if (!canSignal(socket, to, io) || !description) {
        return;
      }
    ...
    socket.on("signal:candidate", ({ to, candidate } = {}) => {
      if (!canSignal(socket, to, io) || !candidate) {
        return;
      }
    ...
    socket.on("control:stream", (payload = {}, ack = () => {}) => {
      ...
      hostSocket
        .timeout(config.controlStreamTimeoutMs)
...
export async function startBabyCamServer(options = {}) {
  const service = createBabyCamServer(options);
  await service.start();
  return service;
}
...
function resolveServerConfig(options) {
  return {
    ...
    iceServers: parseIceServers(options.iceServers ?? process.env.ICE_SERVERS),
    controlStreamTimeoutMs: Math.max(
      5000,
      Number(options.controlStreamTimeoutMs || process.env.CONTROL_STREAM_TIMEOUT_MS || 35000)
    )
  };
}

function logStartup(config) {
  const primaryOrigin = config.publicBaseUrl || `http://localhost:${config.port}`;
  console.log(`\nBabyCam corriendo en ${primaryOrigin}`);

  for (const url of getLocalUrls(config.port)) {
    console.log(`LAN: ${url}`);
  }
...
function leaveCurrentRoom(socket, rooms, io) {
...
function canSignal(fromSocket, toSocketId, io) {
